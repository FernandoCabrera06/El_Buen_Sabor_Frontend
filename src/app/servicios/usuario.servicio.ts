...
export class UsuarioServicio {
  ...
  getUsuariosFromDataBase():any{
    return this.http.get("http://localhost:8080/usuario/listarUsuarios").pipe(
      map( usuariosData => usuariosData));
  }

  //busca un usuario por el id
  getUsuarioEnBaseDatosXId(idx:string){
    return this.http.get("http://localhost:8080/usuario/listarUsuarioXId/" + idx).pipe(
      map( usuarioEncontrado => usuarioEncontrado));
  }

  //login de un usuario
  getLoginUsuario(usuario:string,clave:string){
    return this.http.get("http://localhost:8080/usuario/login?usuario="+usuario+"&clave="+clave).pipe(
      map( usuarioEncontrado => usuarioEncontrado as Usuario));
  }
